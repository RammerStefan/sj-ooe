...
- **Zweiter Kommentator (`Bertl`) + Fun Facts:** frei erfundener Co-Kommentator (NICHT Goldberger – Persönlichkeitsrechte!), eigene tiefere TTS-Stimme, sagt zu jedem Austragungsort einen Fun Fact (`FACTS` je Schanze) zu Bewerbs-/Sprungbeginn. Im Menü abschaltbar (`facts`).
- **Hinterzimmer-Tricks im Karrieremodus** (`TRICKS`, ab 3 Bewerben via `tricksUnlocked()`): übergroßer Anzug (+10 % Auftrieb), Spezialsaft (+alle Werte), Wundersaft (+Anlauf) – greifen REAL in `playerMods()`/Physik ein. Jede Option zeigt Auftriebs-/Boost-Gewinn und Erwischchance; Gesamtrisiko im Tricks-Screen. Erwischen (`rollTrickCatch`, ~mittel) → Disqualifikation + 1 Bewerb Sperre (`_banEventIdx`); gesperrte Menschen sitzen den nächsten Bewerb aus. Fiktive Namen, keine realen Personen. Pro Mensch wählbar (Hot-Seat).

- **Eigene Sprachaufnahmen (`Clips` + `AUDIO_MANIFEST`):** Statt Gerätestimme kann der Nutzer eigene MP3s einsprechen. Schema `audio/<key>/<key>-NN.mp3`, pro Aktion mehrere Varianten (Zufallsauswahl). `AUDIO_MANIFEST` gibt die Variantenanzahl je Key an. Schalter in den Einstellungen (`clips`); fehlt eine Datei oder ist der Schalter aus, fällt jede Ansage automatisch auf TTS zurück (`Andi.say`→`_tts`, `Bertl.sayFact`→`_ttsFact`). Keys: start, takeoffPerfect/Good/Poor, flightHigh/Low, overK, overHS, telemark, landOk, crash, record, win, podium, caught, fact_<hillId>. Vollständige Sprechtexte in `AUFNAHMEN.md`.
